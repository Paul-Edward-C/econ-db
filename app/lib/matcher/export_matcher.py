...
class Export_matcher:
    def __init__(self, mapping_path, keep_list, category_name):
        ...
        self.freq_lookup_table = {"M": ["Monthly"]}
    ...
    def match(self, country, freq):
        ...
        for index in tqdm(mapping.index, desc=f"{country} processing mapping", unit="mapping"):
            ...
            result = pd.DataFrame(
                {"dataname": data_columns.tolist(), "fuzzscore": fuzzscore_list, "cosscore": cosscore_list}
            ).sort_values(by=["cosscore", "fuzzscore"], ascending=False)
        ...
        for duplicate in duplicated_dataname_u:
            duplicate_df = all_result.loc[all_result["dataname"] == duplicate].sort_values(
        ...
        for duplicate in duplicated_mappingname_u:
            duplicate_df = all_result.loc[all_result["mappingname"] == duplicate].sort_values(
                by="overallscore", ascending=False
            )
            all_result.drop(duplicate_df.index[1:], inplace=True)
        ...
        output_path = (
            f"db/{country.lower()}/export/{freq.lower()}/"
            f"{dt.strftime(dt.now().date(), '%Y%m%d')} {country} match output.xlsx"
        )

...
class Mapping_Generator:
    ...
    def create_mapping(self):
        ...
        iter_columns = template.columns[
            : list(template.columns).index(self.sep)
        ]
        iter_df = template[iter_columns].dropna(how="all")

        tier_df = (
            template[
                template.columns[list(template.columns).index(self.sep) :]
            ]
            .set_index(self.sep)
            .dropna(how="any")
        )
    ...
    def create_tierdf(self, source, length):
        ...
        for index in range(len(source.index)):
            ...
            if current_order >= next_order:
                temp.loc[
                    len(temp),
                    [i for i in range(length, current_order + 1)],
                ] = [col_dict[i] for i in range(length, current_order + 1)]
        return temp
